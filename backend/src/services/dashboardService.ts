...
class DashboardService {
   ...
   */
  async getDashboardOverview(userId: string, userRole: string, options: any = {}) {
   ...
   */
  async getTeamPerformance(userId: string, userRole: string, options: any = {}) {
    const { dateRange = '30', projectId } = options;
   ...
   */
  async getProductivityAnalytics(userId: string, userRole: string, options: any = {}) {
    const { dateRange = '30', projectId } = options;
    ...
    try {
      ...
      const priorityGroups: Record<string, { totalHours: number; taskCount: number }> = {};
   ...
   */
  getDateFilter(dateRange: string) {
    const days = parseInt(dateRange);
   ...
   */
  getProjectFilterConditions(userId: string, userRole: string, projectId?: string) {
  ...
  getProjectFilter(userId: string, userRole: string, projectId?: string | null) {
    const { conditions } = this.getProjectFilterConditions(userId, userRole, projectId || undefined);
    return (conditions.id as string | undefined) || null;
  }

  /**
   * Helper method to check if user has access to projects based on role
   */
  async getUserAccessibleProjects(userId: string, userRole: string, projectId: string | null = null) {
   ...
   */
  async getTimeStatistics(userId: string, userRole: string, dateFilter: any, projectId?: string) {
    try {
      const filters = {
        ...dateFilter,
        ...(projectId && { projectId })
      ...
      const totals = totalHours as { totalHours: number; billableHours?: number; totalEntries: number };

      // Calculate productivity metrics
      const totalDays = this.getDaysInDateRange(dateFilter);
      const avgHoursPerDay = totalDays > 0 ? (totals.totalHours / totalDays) : 0;
      ...
      return {
        ...
        billablePercentage: totals.totalHours > 0 ? 
          Math.round(((totals.billableHours || 0) / totals.totalHours) * 100) : 0,
   ...
   */
  async getTimeDistribution(userId: string, userRole: string, dateFilter: any, projectId?: string) {
    try {
      const filters = {
        ...dateFilter,
        ...(projectId && { projectId })
   ...
   */
  async getTeamProductivity(userId: string, userRole: string, options: any = {}) {
    try {
      const { dateRange = '30', projectId } = options;
      const dateFilter = this.getDateFilter(dateRange);
   ...
   */
  async getProjectAnalytics(projectId: string, userId: string, userRole: string, options: any = {}) {
    try {
   ...
   */
  getDaysInDateRange(dateFilter: any) {
   ...
   */
  async getTeamTimeAnalytics(filters: any) {
    try {
   ...
   */
  async getTeamTaskAnalytics(filters: any) {
    try {
   ...
   */
  async getTeamActivityAnalytics(filters: any) {
    try {
      ...
      const activityByUser = activities.reduce((acc: Record<string, any>, activity: any) => {
      ...
      return {
        ...
        mostActiveUsers: (Object.values(activityByUser) as Array<{ activities: number }>)
   ...
   */
  calculateUtilizationRate(timeReport: any[]) {
   ...
   */
  calculateActivityTrend(activities: any[]) {
  ...
  async getProjectOverview(projectId: string, dateFilter: any) {
    return this.getProjectStatistics('', 'admin', dateFilter, projectId);
  }

  async getProjectTaskAnalytics(projectId: string, dateFilter: any) {
    return this.getTaskStatistics('', 'admin', dateFilter, projectId);
  }

  async getProjectTimeAnalytics(projectId: string, dateFilter: any) {
    return this.getTimeStatistics('', 'admin', dateFilter, projectId);
  }

  async getProjectTeamPerformance(projectId: string, dateFilter: any) {
    const days = this.getDaysInDateRange(dateFilter);
    return this.getTeamPerformance('', 'admin', { dateRange: String(days), projectId });
  }
