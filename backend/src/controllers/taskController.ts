...
const normalizeTaskPayload = (task) => {
  ...
  if (Array.isArray(task.comments)) {
    normalized.comments = task.comments.map((c) => normalizeCommentPayload(c));
  }
...
class TaskController {
  ...
  async getAllTasks(req, res) {
    try {
      ...
      const payload = {
        ...result,
        tasks: Array.isArray(result.tasks) ? result.tasks.map((t) => normalizeTaskPayload(t)) : result.tasks,
      };

      res.status(200).json(formatApiResponse(payload, 'Tasks retrieved successfully'));
  ...
  async getTaskById(req, res) {
    try {
      ...
      res.status(200).json(formatApiResponse(normalizeTaskPayload(task), 'Task retrieved successfully'));
  ...
  async createTask(req, res) {
    try {
      ...
      res.status(201).json(formatApiResponse(normalizeTaskPayload(task), 'Task created successfully'));
  ...
  async updateTask(req, res) {
    try {
      ...
      res.status(200).json(formatApiResponse(normalizeTaskPayload(task), 'Task updated successfully'));
  ...
  async addComment(req, res) {
    try {
      ...
      res.status(201).json(formatApiResponse(normalizeTaskPayload(task), 'Comment added successfully'));
  ...
  async updateComment(req, res) {
    try {
      ...
      res.status(200).json(formatApiResponse(normalizeCommentPayload(comment), 'Comment updated successfully'));
  ...
  async getUserTasks(req, res) {
    try {
      ...
      res.status(200).json(formatApiResponse({ tasks: (tasks || []).map((t) => normalizeTaskPayload(t)) }, 'User tasks retrieved successfully'));
  ...
  async getTasksByProject(req, res) {
    try {
      ...
      res.status(200).json(formatApiResponse({ tasks: (tasks || []).map((t) => normalizeTaskPayload(t)) }, 'Project tasks retrieved successfully'));
  ...
  async getTaskComments(req, res) {
    try {
      ...
      return res.status(200).json(formatApiResponse((comments || []).map((c) => normalizeCommentPayload(c)), 'Task comments retrieved successfully'));
    } catch (error) {
