...
router.put('/:id/status',
  ...
  async (req, res) => {
    ...
    // Normalize 'in-progress' -> 'in_progress' to match backend enum
    if (req.body && req.body.status === 'in-progress') {
...
// Compatibility: unassign task via /api/tasks/:id/unassign
router.put('/:id/unassign',
  authenticateToken,
  async (req, res) => {
    req.body = { ...(req.body || {}), assignedTo: null };
...
// Compatibility: update task priority via /api/tasks/:id/priority with { priority }
router.put('/:id/priority',
  authenticateToken,
  async (req, res) => {
    // Normalize legacy 'urgent' -> 'critical'
    if (req.body && req.body.priority === 'urgent') {
...
// Compatibility: duplicate a task via /api/tasks/:id/duplicate
router.post('/:id/duplicate',
  ...
  async (req, res) => {
    try {
      const { id } = req.params;
      const { user } = req;
      const sourceTask = await taskService.getTaskById(id, user.id, user.role);

      if (!sourceTask) {
        return res.status(404).json({ success: false, message: 'Task not found' });
      }

      const targetProjectId = req.body?.projectId || sourceTask.projectId;
      const duplicatedTask = await taskService.createTask(
        {
...
// Compatibility route expected by some clients: PUT /api/tasks/:id/comments/:commentId
...
// Compatibility route expected by some clients: DELETE /api/tasks/:id/comments/:commentId
